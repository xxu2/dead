...
#     include "define.h"
     ...
     &                                         / SPC_HEAT_DRY_AIR
      ...
      INTEGER, PARAMETER   :: NBINS_WND_FRC    = 100
...
!
      ...
      ! added for DEAD scale factors
      USE ADJ_ARRAYS_MOD,    ONLY : EMS_SF
      USE ADJ_ARRAYS_MOD,    ONLY : IDADJ_EDEAD_S
      USE ADJ_ARRAYS_MOD,    ONLY : IDADJ_EDEAD_UT
      USE ADJ_ARRAYS_MOD,    ONLY : IDADJ_EDEAD_EV
      USE ADJ_ARRAYS_MOD,    ONLY : IDADJ_EDEAD_CF
      USE ADJ_ARRAYS_MOD,    ONLY : IS_DEAD_EMS_ADJ
...
#     include "CMN_SIZE"     ! Size parameters
      ...
      REAL*8,  INTENT(INOUT), OPTIONAL :: DSRC_WF(IIPAR,NDSTBIN,4)
      ...
      !================================================================
      ! Local variables added for DEAD adjoint (xxu, 1/13/15)
      !================================================================
    
      ! Saltation constant Whi79 p. 4648, MaB97 p. 16422
      REAL*8,  PARAMETER   :: CST_SLT = 2.61d0

      ! Ratio of wind friction threshold to wind friction
      REAL*8               :: U_S_RAT

      ! Mass fraction clay limited to 0.20
      REAL*8               :: MSS_FRC_CLY_VLD

      ! Natural log of 10
      REAL*8               :: LN10

      ! Exponential value for wind fricton velocity
      REAL*8               :: WND_FRC_EV
      ...
      ! Counting index 
      INTEGER              :: SRC_IDX  ! for source bins
      INTEGER              :: MM       ! for scaling factor time-groups

      ! Parameters scaled
      REAL*8               :: SRCE_FUNC_SLICE_SCALED(IIPAR)
      REAL*8               :: WND_FRC_THR_SLT_SCALED(IIPAR)
      REAL*8               :: WND_FRC_EV_SCALED(IIPAR)

      ! Parameters for Weibull PDF calculations
      LOGICAL, PARAMETER   :: DO_SUBSCALE_WIND = .TRUE.
      REAL*8               :: WND_FRC_CUTOFF(2)
      REAL*8               :: WND_FRC_STEPS(0:NBINS_WND_FRC)
      REAL*8               :: WND_FRC_WEIBULL(0:NBINS_WND_FRC)
      INTEGER              :: NBIN_LIMITS(2)
      INTEGER              :: IWB, IWFS
      REAL*8               :: FLX_MSS_HRZ_SLT_WB(0:NBINS_WND_FRC)
      REAL*8               :: FLX_MSS_HRZ_SLT_DWB(0:NBINS_WND_FRC,3)
      REAL*8               :: FLX_MSS_HRZ_SLT_DTTL(IIPAR,3)
      REAL*8               :: DELTA_WB

      !=================================================================
      ! DST_MBL begins here!
      !=================================================================
      ! WRITE(*,'(A)') ' DST_MBL: begins here'
     ...
     &    VAI_DST_SLICE)       ! I [m2 m-2] Vegetation area index, one-sided
      ...
      if (mbl_nbr == 0) then
!        print *,' no mobilisation candidates', LAT_IDX
        goto 737
      endif
...
!-----------------------------------------------------------------------------
! Prior to 1/13/15:
! We now calculate the saltation flux directly, instead of by calling
! below subroutine:
...
!-----------------------------------------------------------------------------
      ...
      ! Now copy code from routine FLX_MSS_HRZ_SLT_TTL_WHI79_GET here:
      ! (xxu, 1/13/15)

      !================================================================
      ! Now all needed parameters are ready, initialize ...
      !================================================================
      FLX_MSS_HRZ_SLT_TTL = 0.0D0
      FLX_MSS_HRZ_SLT_DTTL = 0.0D0
      DST_SLT_FLX_RAT_TTL(:) = 0.0D0
      FLX_MSS_VRT_DST_TTL(:) = 0.0D0

      LN10       = LOG(10.0D0)
      WND_FRC_EV = 3.0D0
      IF ( PRESENT(DSRC_WF) ) DSRC_WF = 0.0D0

      ! Determine group (temporal) (xxu, dkh, 01/13/12, adj32_011) 
      MM = GET_SCALE_GROUP()
      IF ( LAT_IDX==1 ) WRITE(6,*) '    - USE SCALE_GROUP', MM

      ! Copy original source function
      SRCE_FUNC_SLICE_SCALED(:) = SRCE_FUNC_SLICE(:)

      ! Copy the original wind friction threshold
      WND_FRC_THR_SLT_SCALED(:) = WND_FRC_THR_SLT(:)

      ! Copy the original exponential power
      WND_FRC_EV_SCALED(:) = WND_FRC_EV

      ! If do DEAD ajoint, apply the adjoint scaling factor
      IF ( LADJ_EMS .and. IS_DEAD_EMS_ADJ) THEN
 
         ! Update source function
         IF ( IDADJ_EDEAD_S > 0 )
     &   SRCE_FUNC_SLICE_SCALED(:) = SRCE_FUNC_SLICE(:)
     &      * EMS_SF(:,LAT_IDX,MM,IDADJ_EDEAD_S)

         ! Update wind frc. threshold
         IF ( IDADJ_EDEAD_UT > 0 )
     &   WND_FRC_THR_SLT_SCALED(:) = WND_FRC_THR_SLT(:)
     &      * EMS_SF(:,LAT_IDX,MM,IDADJ_EDEAD_UT)

         ! Update the exponential value of WND_FRC_SLT
         IF ( IDADJ_EDEAD_EV > 0 )
     &   WND_FRC_EV_SCALED(:) = WND_FRC_EV
     &      * EMS_SF(:,LAT_IDX,MM,IDADJ_EDEAD_EV)
      ...
      IF ( FLG_MBL_SLICE(I) ) THEN 
         ...
         ! Calculate the wind speed Weibull distribution with 1-tail 1%
         ! cutoff
         CALL GET_WND_FRC_WEIBULL( WND_RFR(I),                ! I
     &                             WND_FRC_SLT(I),            ! I
     &                             WND_FRC_THR_SLT_SCALED(I), ! I 
     &                             WND_FRC_CUTOFF,            ! O  
     &                             WND_FRC_STEPS,             ! O
     &                             WND_FRC_WEIBULL,           ! O
     &                             NBIN_LIMITS )              ! O
 
         ! If not considering the sub-scale wind 
         IF ( .NOT. DO_SUBSCALE_WIND ) THEN 

            ! Zerolize weibull variables
            WND_FRC_CUTOFF = 0.0D0
            WND_FRC_STEPS = 0.0D0
            WND_FRC_WEIBULL = 0.0D0
            NBIN_LIMITS = -1

            ! If the wind friction speed over the threshold
            IF ( WND_FRC_SLT(I) > WND_FRC_THR_SLT_SCALED(I) ) THEN 

               NBIN_LIMITS(1) = 0
               NBIN_LIMITS(2) = 1
               WND_FRC_STEPS(0) = WND_FRC_THR_SLT_SCALED(I)
               WND_FRC_STEPS(1) = WND_FRC_SLT(I)
               WND_FRC_WEIBULL(0) = 0.0D0
               WND_FRC_WEIBULL(1) = 2.0D0 
     &            / ( WND_FRC_SLT(I) - WND_FRC_THR_SLT_SCALED(I) )

            ENDIF
         
         ENDIF

         ! velocity PDF has portion larger than the saltating threshold  
         IF ( NBIN_LIMITS(1) >= 0 ) THEN 

            ! Initialize
            FLX_MSS_HRZ_SLT_WB = 0.0D0
            FLX_MSS_HRZ_SLT_DWB = 0.0D0

            ! Constants of Cs * rho / g
            CST_SLT01 = CST_SLT * DNS_MDP(I) / GRV_SFC

            ! Loop for wind PDF integration
            DO IWB = NBIN_LIMITS(1), NBIN_LIMITS(2)

               ! Ratio of wind friction threshold to wind friction
               !U_S_RAT = WND_FRC_THR_SLT_SCALED(I) / WND_FRC_SLT(I)
               U_S_RAT = WND_FRC_THR_SLT_SCALED(I) / WND_FRC_STEPS(IWB)

               ! Whi79 p. 4648 (19), MaB97 p. 16422 (28)
               FLX_MSS_HRZ_SLT_WB(IWB) 
     &            = WND_FRC_STEPS(IWB)**WND_FRC_EV_SCALED(I)
     &            * (1.0D0-U_S_RAT) * (1.0D0+U_S_RAT)
     &            * (1.0D0+U_S_RAT) 

               IF ( PRESENT( DSRC_WF ) ) THEN

                  ! For calculating the weighting function w.r.t. exp-order
                  ! See Xu's dissertation (xxu, 1/15/15)
                  FLX_MSS_HRZ_SLT_DWB(IWB,2) 
     &               = FLX_MSS_HRZ_SLT_WB(IWB) 
     &               * LOG(WND_FRC_STEPS(IWB))

                  ! For calculating the weighting function w.r.t. u_*t
                  ! See Xu's dissertation (xxu, 1/15/15)
                  FLX_MSS_HRZ_SLT_DWB(IWB,3)
     &               = WND_FRC_STEPS(IWB)**(WND_FRC_EV_SCALED(I)-1.0D0)
     &               * (1.0D0 - 2.0*U_S_RAT - 3.0*U_S_RAT*U_S_RAT) 
               ENDIF

            ENDDO ! IWB

            DO IWB = NBIN_LIMITS(1), NBIN_LIMITS(2) - 1 

               DELTA_WB = (WND_FRC_STEPS(IWB+1)-WND_FRC_STEPS(IWB))*0.5

               FLX_MSS_HRZ_SLT_TTL(I) ! [kg m-1 s-1]
     &            = FLX_MSS_HRZ_SLT_TTL(I) 
     &            + DELTA_WB
     &            * SUM( FLX_MSS_HRZ_SLT_WB(IWB:IWB+1)  
     &                 * WND_FRC_WEIBULL(IWB:IWB+1) )

               ! For calculating the weighting functions
               IF ( PRESENT( DSRC_WF ) ) THEN 
               DO IWFS = 2, 3

                  FLX_MSS_HRZ_SLT_DTTL(I,IWFS)
     &               = FLX_MSS_HRZ_SLT_DTTL(I,IWFS) 
     &               + DELTA_WB
     &               * SUM( FLX_MSS_HRZ_SLT_DWB(IWB:IWB+1,IWFS)
     &                    * WND_FRC_WEIBULL(IWB:IWB+1) )
            ...
            ! Aplly constants
            FLX_MSS_HRZ_SLT_TTL(I) = FLX_MSS_HRZ_SLT_TTL(I) * CST_SLT01
            FLX_MSS_HRZ_SLT_DTTL(I,:) = FLX_MSS_HRZ_SLT_DTTL(I,:) 
     &                                * CST_SLT01
...
!-----------------------------------------------------------------------------
     ...
     &         * LND_FRC_MBL_SLICE(I)       ! [frc] Bare ground fraction
     &         * FLX_MSS_FDG_FCT            ! [frc] Global mass flux tuning
     &         * SRCE_FUNC_SLICE_SCALED(I)  ! GOCART source function
...
!-----------------------------------------------------------------------------
! Prior to 1/13/15:
! We now calculate the vertical flux directly, instead of by calling
! below subroutine:
...
!-----------------------------------------------------------------------------
            ...
            ! Copy from FLX_MSS_VRT_DST_TTL_MAB95_GET (xxu, 1/13/15)

            ! 19990603: fxm: Dust production is EXTREMELY sensitive to
            ! this parameter, which changes flux by 3 orders of
            ! magnitude
            ! in 0.0 < mss_frc_cly < 0.20
            MSS_FRC_CLY_VLD = MIN(MSS_FRC_CLY_SLICE(I),0.2D0)  ! [frc]

            DST_SLT_FLX_RAT_TTL(I) =           ! [m-1]
     &         100.0D0 * EXP(LN10*(13.4D0*MSS_FRC_CLY_VLD-6.0D0))
                                                     ! MaB95 p. 16423
                                                     ! (47)

            FLX_MSS_VRT_DST_TTL(I) =           ! [kg M-1 s-1]
     &         FLX_MSS_HRZ_SLT_TTL(I) * DST_SLT_FLX_RAT_TTL(I)
...
!-----------------------------------------------------------------------------
      ...
      ! Copy from FLX_MSS_VRT_DST_PRT (xxu, 1/13/15)

            ! Loop over source & sink indices
            DO M = 1, NDSTBIN
            DO SRC_IDX = 1, DST_SRC_NBR

               FLX_MSS_VRT_DST(I,M) = ! [kg m-2 s-1]
     &              FLX_MSS_VRT_DST(I,M)
     &              + OVR_SRC_SNK_MSS(SRC_IDX,M)
     &              * FLX_MSS_VRT_DST_TTL(I)

               IF ( PRESENT( DSRC_WF ) ) THEN 
               DO IWFS = 2, 3

                  DSRC_WF(I,M,IWFS) = DSRC_WF(I,M,IWFS) 
     &                + OVR_SRC_SNK_MSS(SRC_IDX,M)
     &                * FLX_MSS_HRZ_SLT_DTTL(I,IWFS)
...
!------------------------------------------------------------------------------
! Commented out by xxu when developing subgrid wind 
! (xxu, 1/22/15)
!      !=================================================================
...
!------------------------------------------------------------------------------
     ...
     &              TM_ADJ * Q_DST_TND_MBL(I,M)
               ...
               ! If return weighting function
               IF ( PRESENT(DSRC_WF) ) THEN 

                 ! w.r.t. erodibility
                 DSRC_WF(I,M,1) = DSRC(I,M) * SRCE_FUNC_SLICE(I)
     &                          / SRCE_FUNC_SLICE_SCALED(I) 

                 ! w.r.t. exponential order
                 DSRC_WF(I,M,2) = DSRC_WF(I,M,2) * GET_AREA_M2(LAT_IDX)
     &                          * TM_ADJ
     &                          * WND_FRC_EV
 
                 ! w.r.t. wind frc velocity threshold
                 DSRC_WF(I,M,3) = DSRC_WF(I,M,3) * GET_AREA_M2(LAT_IDX)
     &                          * TM_ADJ 
     &                          * WND_FRC_THR_SLT(I)
         ...
         ENDIF ! IF ( NBIN_LIMITS(1) >= 0 )

      ENDIF ! IF ( FLG_MBL_SLICE(I) )
      ENDDO ! I
...
!
!********************************************************************************
! Subroutine GET_SCALE_GROUP determines which predifined scaling index
! corresponds
...
#     include "CMN_SIZE" ! Size stuff
      ...
      ELSE
         ...
         ELSE
            ...
            ! Index is the current time divided by the group length,
            ! plus one
...
!------------------------------------------------------------------------------
      ...
      REAL*8 FUNCTION GAMMLN( XARG )
!
!******************************************************************************
! Function GAMMLN returns the value of the natural logarithm of the
! gamma function. 
! (xxu, 1/19/15)
! The argument XARG must be real and positive
! Original code from 'Numerical Recipes'
!******************************************************************************
!
      ...
      REAL*8, PARAMETER      :: GAMMAF_EPS = 1.0D-10
      REAL*8, PARAMETER      :: GAMMAF_FPF = 5.5D0
      REAL*8                 :: COF(6), STP, SER
      REAL*8                 :: X, Y, TMP
      INTEGER                :: J
     ...
     &           -0.5395239384953D-5  /    
      ...
      ! Check for bad input
      IF ( XARG <= 0.0D0 ) THEN
         CALL ERROR_STOP( 'XARG can not be negative', 
     &                    'GAMMLN (dust_dead_mod.f)' )

      ENDIF

      IF ( ABS( XARG - 1.0D0 ) < GAMMAF_EPS ) THEN 
         CALL ERROR_STOP( 'XARG is too close to one',
     &                    'GAMMLN (dust_dead_mod.f)' )
...
!------------------------------------------------------------------------------
      ...
      SUBROUTINE GET_WND_FRC_WEIBULL( WND_RFR,               ! I
     &                                WND_FRC_SLT,           ! I
     &                                WND_FRC_THR_SLT,       ! I 
     &                                WND_FRC_CUTOFF,        ! O  
     &                                WND_FRC_STEPS,         ! O
...
!
!******************************************************************************
! Subourtine GET_WND_FRC_WEIBULL calculate the Weibull wind distribution
! (xxu, 1/21/15)
!  - Arguments:
!    WND_RFR: 10-m wind speed
!    WND_FRC_SLT: wind frc. velocity
!    WND_FRC_THR_SLT: threshold
!    WND_FRC_CUTOFF: lower and upper cutoff of wind speed 
!    NBINS_WND_FRC: number of discrete bins for integration
!    WND_FRC_WEIBULL: Weibull density function for each discrete bin
!******************************************************************************
! 
      ! Arguments
      REAL*8, INTENT(IN)    :: WND_RFR
      REAL*8, INTENT(IN)    :: WND_FRC_SLT
      REAL*8, INTENT(IN)    :: WND_FRC_THR_SLT
      REAL*8, INTENT(OUT)   :: WND_FRC_CUTOFF(2)
      REAL*8, INTENT(OUT)   :: WND_FRC_STEPS(0:NBINS_WND_FRC)
      REAL*8, INTENT(OUT)   :: WND_FRC_WEIBULL(0:NBINS_WND_FRC)
      INTEGER, INTENT(OUT)  :: NBIN_LIMITS(2)

      ! Local variables
      REAL*8                :: RAT_U10_USTAR
      REAL*8                :: WND_RFR_THR
      REAL*8                :: KW, CW, INV_KW
      REAL*8                :: GAMMA1, CUTOFF
      REAL*8                :: U10STEP

      INTEGER               :: I

      !================================================================
      ! GET_WND_FRC_WEIBULL begins here!
      !================================================================

      ! Ratio of 10-m wind speed to the wind friction speed
      RAT_U10_USTAR = WND_RFR / WND_FRC_SLT

      ! Calculate the Weibull parameters based on the 10-m wind 
      ! Ref. equation (20) [Justus et al 1978]
      KW = 0.94D0 * DSQRT( WND_RFR )

      ! Ref. equation (16) [Justus et al 1978]
      INV_KW = 1.0D0 / KW
      GAMMA1 = GAMMA( 1.0D0 + INV_KW )
      CW     = WND_RFR / GAMMA1

      ! Define boundaries based on single-tail 1% confidence level
      CUTOFF = 0.01
      WND_FRC_CUTOFF(1) = CW * ( - LOG(1.0D0-CUTOFF) ) ** INV_KW
      WND_FRC_CUTOFF(2) = CW * ( - LOG(CUTOFF) ) ** INV_KW

      ! Discretize the PDF
      U10STEP = ( WND_FRC_CUTOFF(2) - WND_FRC_CUTOFF(1) )
     &        / REAL(NBINS_WND_FRC,8)

      DO I = 1, NBINS_WND_FRC 

         WND_FRC_STEPS(I) = WND_FRC_CUTOFF(1) + U10STEP * I
         WND_FRC_WEIBULL(I) = KW / CW 
     &                      * ( WND_FRC_STEPS(I) / CW ) ** ( KW-1.0D0 )
     &                      * EXP( - ( WND_FRC_STEPS(I) / CW ) **  KW )
 
      ENDDO

      ! Now scale the Weibull PDF and bin info from 10-m wind to surface
      ! wind friction speed. Here assume U10 is linear to the u_star
      WND_FRC_CUTOFF  = WND_FRC_CUTOFF / RAT_U10_USTAR
      WND_FRC_STEPS   = WND_FRC_STEPS  / RAT_U10_USTAR
      WND_FRC_WEIBULL = WND_FRC_WEIBULL * RAT_U10_USTAR

      !================================================================
      ! Now get the portion of the PDF on the right side of threshold
      !================================================================

      ! The threshold is beyound the upper bound of u_star
      IF ( WND_FRC_CUTOFF(2) <= WND_FRC_THR_SLT ) THEN 

         ! Return nagive value for 
         NBIN_LIMITS = -1
         RETURN

      ! The threhold is smaller than the lower bound of u_satr
      ELSE IF ( WND_FRC_CUTOFF(1) >= WND_FRC_THR_SLT ) THEN 
 
         ! Use the full 98% of PDF
         NBIN_LIMITS(1) = 0
         NBIN_LIMITS(2) = NBINS_WND_FRC
         RETURN

      ! The threshold is located between the bounds
      ELSE

         ! Retain the upper bound
         NBIN_LIMITS(2) = NBINS_WND_FRC

         ! Find the location of threshold value
         DO I = 0, NBINS_WND_FRC
            IF ( WND_FRC_STEPS(I) < WND_FRC_THR_SLT ) NBIN_LIMITS(1) = I
         ENDDO

         ! Replace the lower bound with threshold
         WND_FRC_STEPS( NBIN_LIMITS(1) ) = WND_FRC_THR_SLT
         WND_RFR_THR = WND_FRC_THR_SLT * RAT_U10_USTAR
         WND_FRC_WEIBULL( NBIN_LIMITS(1) ) 
     &      = KW / CW
     &      * ( WND_RFR_THR / CW ) ** ( KW-1.0D0 )
     &      * EXP( - ( WND_RFR_THR / CW ) **  KW )
     &      * RAT_U10_USTAR
      ...
      END SUBROUTINE GET_WND_FRC_WEIBULL
